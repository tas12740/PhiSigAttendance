...
def register_sibling(request):
    if request.method == 'POST':
        ...
        if phone_number is None:
            return HttpResponseBadRequest('You must submit your phone number.')

        preferred_contact = data.get('preferred_contact')
        if preferred_contact is None:
            return HttpResponseBadRequest('You must submit your preferred contact method.')

        pronouns = data.get('pronouns')
        if pronouns is None:
            return HttpResponseBadRequest('You must submit your pronouns.')

        house = data.get('house')
        if house is None:
            return HttpResponseBadRequest('You must submit your house.')

        new_sibling = Sibling(first_name=first_name, last_name=last_name, onyen=onyen, email=email, pledge_class=pledge_class,
                              phone_number=phone_number, preferred_contact=preferred_contact, pronouns=pronouns, house=house)
        try:
            new_sibling.save()
